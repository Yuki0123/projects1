...
class Area(models.Model):
    name = models.CharField(verbose_name='区画', max_length=255)
    field = models.ForeignKey(Field, verbose_name='畑', on_delete = models.CASCADE)
    done = models.BooleanField(verbose_name='終了',default=False)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, null=True, blank=True)
...
class Row(models.Model):
    name = models.CharField(verbose_name='畝', max_length=255)
    area = models.ForeignKey(Area, verbose_name='区画', on_delete = models.CASCADE)
    done = models.BooleanField(verbose_name='終了',default=False)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, null=True, blank=True)
...
class GrowingCrop(models.Model):
    ...
    area = models.ForeignKey(Area, verbose_name='区画', null=True, on_delete = models.SET_NULL)
    row = models.ForeignKey(Row, verbose_name='畝', null=True, on_delete = models.SET_NULL)   
...
class CropManagement(models.Model):
    area = models.ForeignKey(Area,verbose_name='区画', null=True, blank=True, on_delete = models.SET_NULL)
    row = models.ForeignKey(Row,verbose_name='畝', null=True,blank=True, on_delete = models.SET_NULL)
    growing_crop = models.ForeignKey(GrowingCrop, verbose_name='栽培作物',null=True, blank=True, on_delete=models.SET_NULL)
    ...
    def __str__(self) -> str:
        return '{} | {} | {} | {} | {}'.format(self.area,self.row,self.growing_crop,self.title,self.date)
